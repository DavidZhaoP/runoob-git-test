...
//        __p[0], __p[1], __p[2], __p[3], __p[4], __p[5], __p[6], __p[7]);
  ...
  for (const auto& _n : {"lft-leg", "rgt-leg"}) {
...
bool PdNode::__fill_pos_cmd(std::vector<Packet>& pkts) {
  static std::vector<std::string> _s_jnts_leg = {"lft-leg",  "rgt-leg"};
  static std::vector<std::string> _s_jnts_wing = {"lft-wing", "rgt-wing", "pitch", "cog"};
  ...
  int    count = 0;
  bool is_any_valid = false;
  Packet cmd_leg = {bus_id_, node_id_, _s_msg_ids[0], 8, {0}};
  Packet cmd_wing = {bus_id_, node_id_, _s_msg_ids[1], 8, {0}};
//   struct Packet {
...
// };
  ...
  for (const auto& _ns : _s_jnts_leg) {

    auto jnt   = jnts_by_name_[_ns];
    auto param = params_by_name_[_ns];
    ...
    if (jnt->new_command_) {
        ...
        memcpy(cmd_leg.data + offset, &count, sizeof(count));
        // printf("LegNode: [%s] - (%s):\t%05d\n", LEGTYPE_TOSTRING(leg_), JNTTYPE_TOSTRING(type), count);
        jnt->new_command_ = false;
    
      } else {
        cmd_leg.data[offset]     = INVALID_BYTE;
        cmd_leg.data[offset + 1] = INVALID_BYTE;
        cmd_leg.data[offset + 2] = INVALID_BYTE;
        cmd_leg.data[offset + 3] = INVALID_BYTE;
      }
      offset += 4; // Each count stand two bytes.
      
    }
    if (is_any_valid) pkts.push_back(cmd_leg);

   is_any_valid = false; 
   short wing_count = 0;
   offset = 0;
  for (const auto& _n : _s_jnts_wing) {
    ...
    if (jnt->new_command_) {
        ...
        // if ((LegType::FL == leg_) && (JntType::HIP == type))
        //   printf("LegNode: [%d] - (%d): %+01.04f\n", leg_, type, jnt_cmds_[type][0]);
        wing_count = (jnt->joint_command() - param->offset) / param->scale;
        memcpy(cmd_wing.data + offset, &wing_count, sizeof(wing_count));
        // printf("LegNode: [%s] - (%s):\t%05d\n", LEGTYPE_TOSTRING(leg_), JNTTYPE_TOSTRING(type), count);
        jnt->new_command_ = false;
      } else {
        cmd_wing.data[offset]     = INVALID_BYTE;
        cmd_wing.data[offset + 1] = INVALID_BYTE;
      }
      offset += sizeof(wing_count); // Each count stand two bytes.
      
    }
    if (is_any_valid) pkts.push_back(cmd_wing);
